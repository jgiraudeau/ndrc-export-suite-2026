  ...
  const handleSaveUrl = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/student/profile", {
        ...
        body: JSON.stringify({ prestaUrl })
      });
      if (res.ok) {
        // Update local storage
        const newUser = { ...student, prestaUrl };
      ...
      <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row items-center gap-8">
         <div className="flex-1 space-y-2">
            ...
            <div className="flex gap-2 mt-4">
              <input 
                ...
                placeholder="https://la-boutique-ndrc.com"
                value={prestaUrl}
                onChange={(e) => setPrestaUrl(e.target.value)}
              />
              <button 
                onClick={handleSaveUrl}
                disabled={isSaving}
                className="bg-amber-600 text-white px-6 py-3 rounded-xl font-black text-sm flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-lg shadow-amber-100"
              >
         ...
         {prestaUrl && (
            ...
            >
               ...
               <span className="text-[10px] font-black uppercase tracking-widest leading-none">Visiter Boutique</span>
            </a>
         )}
      </div>
