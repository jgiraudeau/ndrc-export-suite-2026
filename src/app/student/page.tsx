...
export default function StudentDashboard() {
  ...
  const badge = calculateBadge(data.progress.acquiredCount, data.progress.totalCount);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-indigo-600 rounded-3xl p-6 text-white shadow-xl shadow-indigo-200 relative overflow-hidden flex flex-col justify-between min-h-[200px]">
                ...
                <div className="relative z-10 mt-6 flex flex-wrap gap-2">
                  <div className={cn(
                    "px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border backdrop-blur-md shadow-lg",
                    badge.color
                  )}>
                    {badge.label}
                  </div>
                  <Link href="/student/wordpress" className="bg-white/10 hover:bg-white/20 backdrop-blur-sm py-1.5 px-3 rounded-full text-[10px] font-bold text-center transition-colors">
                    WordPress
                  </Link>
                  <Link href="/student/prestashop" className="bg-white/10 hover:bg-white/20 backdrop-blur-sm py-1.5 px-3 rounded-full text-[10px] font-bold text-center transition-colors">
                    PrestaShop
                  </Link>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {data.wpUrl ? (
                  ...
                  <div className="w-full border-2 border-dashed border-slate-200 text-slate-400 font-bold text-center py-3 rounded-xl text-sm italic">
                    {/* Section d'Export */}
                    <div className="bg-white rounded-[32px] p-8 shadow-xl shadow-slate-200/50 border border-slate-100 mb-10 overflow-hidden relative group">
                        ...
                        <div className="relative z-10 flex flex-col gap-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    onClick={async () => {
                                        ...
                                        if (exps) {
                                            PDFExportService.generateBadgeExport({
                                                title: "Passeport Professionnel",
                                                studentName: `${data.firstName} ${data.lastName}`,
                                                badge: badge,
                                                items: exps.map(e => ({ title: e.title, description: e.description, status: e.status }))
                                            });
                                        }
                                    }}
                                    className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:bg-indigo-50 hover:border-indigo-100 transition-all group/btn"
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="text-left">
                                            <div className="text-xs font-black text-slate-800 uppercase tracking-wider">Passeport Pro</div>
                                            <div className="text-[10px] text-slate-400 font-bold">{badge.label} inclus</div>
                                        </div>
                                    </div>
                                ...
                                <button 
                                    onClick={async () => {
                                        ...
                                        if (logs) {
                                            PDFExportService.generateBadgeExport({
                                                title: "Épreuve E5B - Dossier",
                                                studentName: `${data.firstName} ${data.lastName}`,
                                                badge: badge,
                                                items: logs.map(l => ({ title: l.content.substring(0, 30) + '...', description: l.content, status: l.isValidated ? "Validé" : "En attente" }))
                                            });
                                        }
                                    }}
                                    className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:bg-amber-50 hover:border-amber-100 transition-all group/btn"
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="text-left">
                                            <div className="text-xs font-black text-slate-800 uppercase tracking-wider">Dossier E5B</div>
                                            <div className="text-[10px] text-slate-400 font-bold">Missions & Preuves</div>
                                        </div>
                                    </div>
                                    <ChevronRight size={16} className="text-slate-300 group-hover/btn:translate-x-1 transition-transform" />
                                </button>
                            ...
                            <button 
                                ...
                                className="w-full flex items-center justify-center gap-3 bg-slate-900 text-white rounded-2xl py-4 font-black text-sm shadow-xl shadow-slate-200 hover:bg-black transition-all active:scale-[0.98]"
                            >
                                <Trophy size={18} className="text-amber-400" />
                                Télécharger le Pack Examen Complet (ZIP)
                            </button>
                        </div>
                    </div>
